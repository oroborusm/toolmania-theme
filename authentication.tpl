...
<div class="formCarrito">
	...
	<!-- <div id="center_column" class="autenticacion"> -->
	<div id="center_column" class="autenticacion">
		{capture name=path}
		...
		{if !isset($email_create)}
			...
			<div class="contenedorLogin">
				<div class="creaCuenta">
					<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="create-account_form" class="formsAtenticacion">
				...
				<div class="accedeCuenta">
					<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="formsAtenticacion">
		...
		{else}
			...
			<!-- crea cuenta -->
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box formularioCrea">
				{$HOOK_CREATE_ACCOUNT_TOP}
				<div class="account_creation creaCuenta">
					<h3 class="page-subheading">{l s='Your personal information'}</h3>
					<div class="srSra">
						<label class="tratoGenero">{l s='Title'}</label>
						{foreach from=$genders key=k item=gender}
							<div class="radio-inline radioSeleccion">
								<label for="id_gender{$gender->id}" class="top">
									<input type="radio" name="id_gender" id="id_gender{$gender->id}" value="{$gender->id}" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == $gender->id}checked="checked"{/if} />
								<span class="generoTexto">{$gender->name}</span>
								</label>
							</div>
						{/foreach}
					</div> 
					...
					<div class="required form-group correo">
						...
						<input type="text" class="is_required validate form-control" data-validate="isEmail" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}" required/>
					</div>
