...
<div class="formCarrito">
	{if !$opc}
		...
		<div id="carrier_area">
			...
			<form class="formRodea" id="form" action="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html':'UTF-8'}" method="post" name="carrier_area">
	{else}
		...
		{else}
			...
			{if !$opc}
					<p class="navegador">
						...
						{if !$is_guest}
							{if $back}
								<a href="{$link->getPageLink('order', true, NULL, "step=1&back={$back}&multi-shipping={$multi_shipping}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="volverCompra">
									<span class="icon-chevron-left"></span>
									{l s='Continue shopping'}
								</a>
							{else}
								<a href="{$link->getPageLink('order', true, NULL, "step=1&multi-shipping={$multi_shipping}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="volverCompra">
									<span class="icon-chevron-left"></span>
									{l s='Continue shopping'}
								</a>
							{/if}
						{else}
							<a href="{$link->getPageLink('order', true, NULL, "multi-shipping={$multi_shipping}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="volverCompra">
								<span class="icon-chevron-left"></span>
								{l s='Continue shopping'}
							</a>
						{/if}
						{if isset($virtual_cart) && $virtual_cart || (isset($delivery_option_list) && !empty($delivery_option_list))}
							<button type="submit" name="processCarrier" class="checkout">
								<span>
									{l s='Proceed to checkout'}
									<span class="icon-chevron-right right"></span>
								</span>
							</button>
						{/if}
					</p>
