...
<div class="formCarrito">
	...
	{if isset($empty)}
		<p class="carritoVacio">{l s='Your shopping cart is empty.'}</p>
	{elseif $PS_CATALOG_MODE}
		<p class="algoAndaMal">{l s='This store has not accepted your new order.'}</p>
	{else}
		...
		<div id="order-detail-content" class="detalleProducto">
			<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
				...
				<tfoot>
					...
					{if $use_taxes}
						{if $priceDisplay}
							<tr class="cart_total_price">
								...
								<td colspan="{$col_span_subtotal}" class="text-right conIva">{if $display_tax_label}{l s='Total products (tax excl.)'}{else}{l s='Total products'}{/if}</td>
					...
					{if $use_taxes && $show_taxes}
						<tr class="cart_total_price">
							<td colspan="{$col_span_subtotal}" class="text-right">
								{if $display_tax_label}{l s='Total (tax excl.)'}{else}{l s='Total'}{/if}
							</td>
					...
					<tr class="cart_total_price">
						...
						{if $use_taxes}
							<td colspan="3" class="priceFinal" id="total_price_container">
		...
		{if ((!empty($delivery_option) AND !isset($virtualCart)) OR $delivery->id OR $invoice->id) AND !$opc}
			<div class="listaDireccion">
				{if !isset($formattedAddresses) || (count($formattedAddresses.invoice) == 0 && count($formattedAddresses.delivery) == 0) || (count($formattedAddresses.invoice.formated) == 0 && count($formattedAddresses.delivery.formated) == 0)}
					{if $delivery->id}
						<div class="contenedorListaDireccion"{if !$have_non_virtual_products} style="display: none;"{/if}>
				...
				{else}
					{foreach from=$formattedAddresses key=k item=address}
						<div class="contenedorListaDireccion"{if $k == 'delivery' && !$have_non_virtual_products} style="display: none;"{/if}>
		...
		<p class="navegador">
			...
			<a
				...
				<span class="icon-chevron-left"></span>{l s='Continue shopping'}
			</a>
		</p>
