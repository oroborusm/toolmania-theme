...
{if !$opc}
	...
	{include file="$tpl_dir./errors.tpl"}
		<form action="{$link->getPageLink($back_order_page, true)|escape:'html':'UTF-8'}" method="post" class="orderAddress">
{else}
...
{/if}
	...
	<div class="addresses">
		<div class="contenedorAddresses">
			<div class="selccionDireccion">
				<div class="address_delivery select form-group selector1">
					<label class="eligeDireccion" for="id_address_delivery">{if $cart->isVirtualCart()}{l s='Choose a billing address:'}{else}{l s='Choose a delivery address:'}{/if}</label>
					...
					<p class="checkboxSelecciona"{if $cart->isVirtualCart()} style="display:none;"{/if}>
					...
					<div class="selccionOtraDireccion">
	...
	{if !$opc}
				<p class="navegador">
					...
					<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="volverCompra">
						<span class="icon-chevron-left"></span>
						{l s='Continue Shopping'}
					</a>
					<button class="checkout" type="submit" name="processAddress" class="button btn btn-default button-medium">
						<span>{l s='Proceed to checkout'}<span class="icon-chevron-right right"></span></span>
					</button>
				</p>
			</form>
	{else}
